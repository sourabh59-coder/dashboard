...
class LineChart extends Component {
  state = {
    options: {
      ...
      xaxis: {
        ...
        categories: [], // Date labels will be populated here
      },
      yaxis: {
        title: {
    ...
    series: [
      {
        ...
        data: [], // Visitor counts per day will be populated here
      },
    ],
  };
  ...
  componentDidMount() {
    ...
    // Process the JSON data to count visitors per day
    const visitorCounts = {}; // Use this to store counts per day

    jsonData.forEach((entry) => {
      // Combine adults, children, and babies to get the total visitors for the day
      const totalVisitors = entry.adults + entry.children + entry.babies;

      // Create a date string in the format 'yyyy-mm-dd' (e.g., '2015-07-01')
      const date = `${entry.arrival_date_year}-${entry.arrival_date_month}-${entry.arrival_date_day_of_month}`;

      if (visitorCounts[date]) {
        visitorCounts[date] += totalVisitors;
      } else {
        visitorCounts[date] = totalVisitors;
      }
    });

    // Extract date labels and visitor counts for chart rendering
  ...
  render() {
    return <Chart options={this.state.options} series={this.state.series} type="line" height={350} />;
  }
}
